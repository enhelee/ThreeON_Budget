...
(function (root, factory) {
  ...
  function fillChipInPlace(aoa, cleaned, ledger, C, corrections) {
    ...
    for (const [code, items] of actByCode) {
      ...
      // 예측모드(집계표 실적 없음) 연예산 배분: 못 나누는 구조적 과목(정기점검·예비품·저장품·고온부품·공구)은
      // raw에 사업 구분정보가 없다. 실적 대신 사업의 연예산(계획) 비례로 지사×과목 raw 총액을 배분 → 총액 맞추고 사업별 추정.
      // 재조정모드(actual0 있음)는 기존 로직(금액매칭·지사합산)을 쓰므로 건드리지 않는다.
      const STRUCT_DIST = new Set(["60909002", "60602015", "20790006", "20790005", "10702001", "20706001"]);
      if (STRUCT_DIST.has(code)) {
        for (const io of new Set(prows.map((p) => p.org))) {
          if (prows.some((p) => p.org === io && p.actual0 > 0)) continue; // 재조정모드 지사는 제외(예측모드만)
          const op = prows.filter((p) => p.org === io && p.annual > 0);
          const tot = op.reduce((s, p) => s + p.annual, 0);
          if (tot <= 0) continue;
          let raw = 0; const mine = [];
          for (const it of items) { if (it.bucket === "gyeongsang") continue; if (nrm(it.org) === io) { raw += it.amount; mine.push(it); } }
          if (!raw) continue;
          for (const p of op) sumByRow[p.r] = (p.annual / tot) * raw; // 연예산 비례 배분(덮어쓰기)
          for (const it of mine) it._structDist = true; // 아래 개별 매칭에서 건너뜀
        }
      }
      ...
      for (const it of items) {
        if (it._structDist) continue; // 예측모드 연예산 배분 완료 → 개별 매칭 생략
