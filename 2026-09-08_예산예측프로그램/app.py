...
from views import (
    ...
    grace_matching_page,  # grace 최신 사업매칭(별도 메뉴, 추가만)
    actuals_upload_page,
    budget_upload_page,
...
# 지표(st.metric) 숫자는 자릿수가 맞춰지도록 고정폭 글꼴 + 표 숫자 정렬을 쓴다(설계도면 테마).
st.markdown("""
<style>
...
CATEGORY_PAGES = {
    "예산 계획 수립": ["예산 계획 업로드"],
    "예산 실적 집계": ["실적 업데이트(zrfm2)", "사업 매칭", "사업 매칭 (grace 최신)", "사업 실적 연결", "계획 대비 실적"],
    "예산 실적 분석": [
...
PAGE_RENDERERS = {
    ...
    ("예산 실적 집계", "사업 매칭 (grace 최신)"): lambda: grace_matching_page.render(),
