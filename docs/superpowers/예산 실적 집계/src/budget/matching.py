...
def match_actuals(plan_df, erp_norm_df, budget_items, pl_items, cap_items,
    ...
    in_budget_idx = [i for i, it in enumerate(items_norm)
                     if it in budget_items and _dept_ok(depts_norm[i])]
    ...
    for key, idxs in groups.items():
        ...
        for i in idxs:
            erp_name = _clean_na(erp["사업명"].iloc[i])
    ...
    sel_item_idx = [i for i, it in enumerate(items_norm) if it in budget_items]
