...
4. **4 통계·내보내기**: 과목별/지사그룹/전년비교 + `{연도}년 {구분}예산_실적.xlsx` 등 다운로드
   ...
   - **팀 연계 패널**(맨 위): `결과 JSON (사업 실적 연결)` → 팀 v2 앱(`../예산예측프로그램_팀공유_v2`) 「예산 실적 집계 → 사업 실적 연결」에
     올리면 손익·자본 매칭 실적이 **예산 표준화·중장기 예산(3·4단계)** 입력이 됩니다. `matched/budget/data CSV`는 연동규격 파일(연도 단일, 원 단위).
     내보내기 전에는 지사·과목 구성을 **전체 선택**으로 두세요(미반영분은 v2 입력에서 빠짐).
...
## 5. 테스트
```bash
py -m pytest -q     # 77 passed (약 4~6분) — 팀 v2 폴더가 형제 경로에 있으면 동료 모듈 교차검증까지 수행
```
