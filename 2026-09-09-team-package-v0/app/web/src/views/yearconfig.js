  ...
  return `
    <div class="p-5 sm:p-6">
      ...
      <div class="mt-4 flex flex-wrap items-end gap-2">
        <label class="text-xs font-semibold text-slate-500">${esc(srcLabel)}<input id="ycAliasSrc" class="control mt-1 block" placeholder="예: 경남지사(양산RPS 태양광)"></label>
        <label class="text-xs font-semibold text-slate-500">${esc(dstLabel)}<input id="ycAliasDst" class="control mt-1 block" placeholder="예: 양산지사"></label>
      ...
      <div class="mt-4 max-h-96 overflow-auto"><table class="w-full"><thead class="table-head">
        ...
        <tbody class="divide-y divide-slate-100">${keys.map(k => seeded.has(k) ? `
          <tr class="bg-slate-50 text-slate-400">
            <td class="table-cell">${esc(k)} <span class="badge bg-slate-200 text-slate-500">기본</span></td>
            <td class="table-cell">${esc(map[k])}</td>
            <td class="table-cell text-right text-xs">수정 불가</td></tr>` : `
          <tr><td class="table-cell font-semibold text-slate-900">${esc(k)}</td>
      ...
      ${keys.length ? "" : `<p class="mt-4 text-xs text-slate-400">등록된 별칭이 없습니다.</p>`}
    </div>`
}
...
function masterTables() {
  ...
  const more = arr => arr.length > 200
    ? `<p class="mt-2 text-xs text-slate-400">앞 200건만 표시합니다 (전체 ${arr.length.toLocaleString()}건).</p>` : ""
  return `
    <div class="grid gap-6 p-5 sm:p-6 xl:grid-cols-2">
      <div><h4 class="text-sm font-bold text-slate-700">예산과목 마스터 (${m.items.length.toLocaleString()}건)</h4>
        ...
        <tbody class="divide-y divide-slate-100">${rows(m.items, ["계정코드", "과목명", "속성"])}</tbody></table></div>${more(m.items)}</div>
      <div><h4 class="text-sm font-bold text-slate-700">부서코드 마스터 (${m.depts.length.toLocaleString()}건)</h4>
        <div class="mt-3 max-h-96 overflow-auto"><table class="w-full"><thead class="table-head">
        <tr><th class="px-3 py-2">부서코드</th><th class="px-3 py-2">부서명</th><th class="px-3 py-2">처지사</th></tr></thead>
        <tbody class="divide-y divide-slate-100">${rows(m.depts, ["부서코드", "부서명", "처지사"])}</tbody></table></div>${more(m.depts)}</div>
    </div>`
}
...
export function renderYearConfig() {
  ...
  <section class="panel overflow-hidden">
    <div class="border-b border-slate-200 p-5 sm:p-6">
      ...
      ${locked ? `<p class="mt-3 text-xs font-semibold text-amber-700">🔒 ${esc(state.year)}년은 마감 상태라 기준정보를 고칠 수 없습니다 — 위 '잠금 해제'를 먼저 누르세요.</p>` : ""}
      <div class="mt-4 flex flex-wrap gap-2">${TABS.map(([k, l]) => btn(k, l)).join("")}</div>
    </div>
    ${body}
    ${savable ? `<div class="border-t border-slate-200 p-5"><button class="btn-primary" data-action="save-yearconfig" ${locked ? "disabled" : ""}>저장</button></div>` : ""}
  </section>`
}
