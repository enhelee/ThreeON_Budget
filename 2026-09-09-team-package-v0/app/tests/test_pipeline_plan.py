import os
import openpyxl
from budget import db as dbm, pipeline_plan, config_store
...
def test_run_plan_pl_and_capital(tmp_path):
    ...
    res_pl = pipeline_plan.run_plan(plan_path, conn, out_dir, "2026", "손익")
    ...
    res_cap = pipeline_plan.run_plan(plan_path, conn, out_dir, "2026", "자본")
    assert os.path.exists(res_cap["output_path"])
    assert res_cap["요약"]["행수"] == 1
    assert res_cap["요약"]["총액"] == 120000.0

    # 지사구성이 그 해 DB 에 자동 생성/재사용되는지 확인
    assert conn.execute(
        "SELECT COUNT(*) FROM dept_config WHERE year='2026'").fetchone()[0] == 24
