...
def test_ingest_and_run_from_db(env):
    ...
    res = pipeline_db.run_actual_db(conn, out, "2023", "손익")
...
def test_override_reassigns_voucher(env):
    ...
    conn, out = env
    base = pipeline_db.run_actual_db(conn, out, "2023", "손익")
...
def test_manual_biz_added_as_plan_row(env):
    ...
    res2 = pipeline_db.run_actual_db(conn, out, "2023", "손익")
    ...
    res3 = pipeline_db.run_actual_db(conn, out, "2023", "손익")
...
def test_learn_from_confirmed_results(env):
    ...
    dbm.add_override(conn, "2023", "손익", int(d3["erp_row_id"]),
                     "화성 옥외배관 도색공사")
    pipeline_db.run_actual_db(conn, out, "2023", "손익")
...
def test_make_files_false_skips_outputs_same_numbers(env):
    ...
    conn, out = env
    fast = pipeline_db.run_actual_db(conn, out, "2023", "손익", make_files=False)
    ...
    full = pipeline_db.run_actual_db(conn, out, "2023", "손익", make_files=True)
...
def test_export_regeneration_does_not_add_run(env):
    ...
    conn, out = env
    base = pipeline_db.run_actual_db(conn, out, "2023", "손익", make_files=False)
