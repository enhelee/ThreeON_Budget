...
def run_actual_db(conn, out_dir, year, budget, new_policy="group",
    ...
    attr_map = dbm.load_item_attr_map(conn, year)          # 마스터: 과목→속성(4종)
    code_to_item = dbm.load_code_to_item(conn, year)       # 마스터: 계정코드→과목명
    deptcode_map = dbm.load_deptcode_map(conn, year)       # 마스터: 부서코드→처지사
